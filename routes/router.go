...
func SetupRoutes(r *gin.Engine) {
	...
	api.Use(middleware.JWTAuthMiddleware()) // ใช้ JWT ตรวจสอบ token ทุก route ในกลุ่มนี้
	{
		...
		{
			api.GET("/work-schedules", controllers.GetWorkSchedules)
			api.POST("/work-schedules", controllers.CreateWorkSchedule)
			api.PUT("/work-schedules/:id", controllers.UpdateWorkSchedule)
			api.DELETE("/work-schedules/:id", controllers.DeleteWorkSchedule)
		}
	}
}
